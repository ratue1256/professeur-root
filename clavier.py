...
def touche_pressee(key):
    global buffer_phrase, dernier_temps
    try:
        if hasattr(key, "char") and key.char:
            buffer_phrase += key.char
            dernier_temps = time.time()
            if len(buffer_phrase) > 100:
...
def verifier_et_nettoyer():
    ...
    if phrase and len(phrase) >= 3:
        ...
        if res and callback_faute:
            ...
            # si c est bon on garde juste le dernier mot
            mots = phrase.split()
            buffer_phrase = mots[-1] if mots else ""

def verifier_pause():
    global buffer_phrase, dernier_temps
    if buffer_phrase.strip() and time.time() - dernier_temps > 0.7:
        verifier_et_nettoyer()
